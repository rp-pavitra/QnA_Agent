...
def main():
    ...
    if uploaded_file:
        if "file_processed" not in st.session_state or st.session_state.get("uploaded_filename") != uploaded_file.name:
            with st.spinner("Loading and processing your document..."):
                file_df, column_names = load_file_main(uploaded_file)
                status = store_file_data_in_db(file_df, column_names)
                schema = get_schema()
                st.session_state["file_processed"] = status and schema
                st.session_state["schema"] = schema
                st.session_state["uploaded_filename"] = uploaded_file.name
        else:
            status = st.session_state["file_processed"]
